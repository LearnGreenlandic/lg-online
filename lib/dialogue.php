...
function lg_dialogue_data($state) {
...
<?php
	foreach (lg_dialogue_data($state) as $e) {
...
function lg_dialogue($state, $which) {
	lg_header($state, 'lg1', 'dialogue/'.$which);
	if ($which === '1') {
		lg_dialogue_1($state);
	}
	else if ($which === '2') {
